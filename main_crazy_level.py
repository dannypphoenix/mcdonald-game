...
def main(leveltoplay=0):
    ...
    player = Player(32, 32); #player.health = -1; #player.jetpack = True
...
##                deltaY /= denom
        ...
        while current_platform < len(platforms):
            ...
            if platform.destroyed:
                if isinstance(platform, SpawnerBlock):
                    g = BlockHider(platform.rect.left, platform.rect.top)
                    g.targets.append(player)
                    entities.add(g)
                    drones.append(g)
                    healthbars.append(g.healthbar)
        ...
        while current_enemy < len(dead_enemies):
            ...
            enemy.timer -= 1
            if enemy.timer <= 0 and enemy.respawn:
